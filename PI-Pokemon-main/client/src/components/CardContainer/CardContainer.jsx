...
import {INCREMENT_PAGE, DECREMENT_PAGE} from "../../redux/actions-type"
//CARD CONTAINER

const CardContainer = () => {
  const pokemons = useSelector((state) => state.alpokemons);                  // LLAMAMOS A TODOS LOS POKEMONS
  const filter = useSelector((state) => state.filter);                     // LLAMAMOS A LA PROPIEDAD FILTER PARA SETEARLA
  const pokemon = useSelector((state) => state.pokemonsName);             // LA CONSTANTE POKEMON ES EL POKEMON QUE BUSCAMOS POR SEARCHBAR
  const currentPGE = useSelector((state) => state.paginado);             // PROPIEDAD PARA SETEAR LA PAGINA ULTIMA PAGINA QUE VISITO EL CLIENTE
